...
主要做了 2 成果：
1. 基于 jest 开发了一个前端自动化测试框架 dwt，结合业务要求，dwt 丰富了 jest 的能力，比如实现零配置，丰富了测试报告（差量，yml，xml，json），失败用例 retry
    - 上传测试报告（为什么自己开发自己的测试报告，jest 官方报告不能满足我们的需求，还有一些数据没有，），生成 epc 规范的测试产物 xml，yml，json
    - 初始化配置，类似 vue-cli，react 脚手架（create-react-app）,自动生成 jest 配置文件，零配置成本
    - 自动跑失败测试用例
2. 测试工具集的收敛：开发了本地 cli，类比 web 的微前端的捏合，，开发了 cli 子插件，背景是部门很大，自动化测试工具很多：单元测试，集成测试，UI 自动化测试，兼容性测试，接口测试，客户端、服务端等自动化测试，外面部门很迷惑，不知道入口在哪，文档分散，所以收敛，需要统一的工具，本地研发了一个 cli，远程在流水线上，开发了蓝盾原子，借助远程蓝盾流水线环境，增强了功能，实现了远程调度
    - 差量测试：拉取 2 个 commit，比如增量测试用例等等
    - 远程调度能力（老旧的大型项目，流水线要跑 1 个小时，本地跑 mac 卡，啥也干不了，所以放到远程去跑）
    - 探索式的研发，走过了很多的弯路，Pkg 将 Node.js 项目打包为可执行文件，开发本地 cli 工具 tguitar，并将 dwt 改造成 tguitar 插件、蓝盾原子 插件，但是 dwt-testend 包基于 puppeteer，selenium，UI 自动化测试需要 chromedriver，600M 的大小
