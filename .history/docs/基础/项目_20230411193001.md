...
1. 基于 jest 开发了一个前端自动化测试框架 dwt，结合业务要求，dwt 丰富了 jest 的能力，比如实现零配置，丰富了测试报告（差量，yml，xml，json），失败用例 retry
    ...
    - 自动跑失败测试用例
2. 测试工具集的收敛：类似微前端的捏合，只不过是 cli，开发了 cli 子插件，背景是部门很大，自动化测试工具很多：单元测试，集成测试，UI 自动化测试，兼容性测试，接口测试，客户端、服务端等自动化测试，外面部门很迷惑，不知道入口在哪，文档分散，所以收敛，需要统一的工具，本地研发了一个 cli，远程在流水线上，开发了蓝盾原子，借助远程蓝盾流水线环境，增强了功能，实现了远程调度
